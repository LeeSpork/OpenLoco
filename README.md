...
Recreational use of JokziLoco is not recommended: there is **no guarantee** that saves/scenarios created in JokziLoco - and objects created for it - will work in OpenLoco, and they could potentially even break in future versions of JokziLoco.
...
These pieces allow you to build dual tracks, allowing trains to pass each other on one tile.
...
It went unused however, as neither track object that shipped with the game used it. But it could easily be enabled by creating a track object with the one-sided track pieces flag enabled
...
This branch also includes my Input​Session changes that convert it to use UTF-8.
